  ...
  return (
    <div className='w-full h-screen max-h-screen text-gray-200 py-2 flex flex-col gap-2 justify-between overflow-y-scroll hidescrollbar'>
        ...
        <div className="flex flex-col gap-2">
          {sidebar_menu_items.map(item=><Link href={item.redirect||""} key={item.name} className={`flex gap-4 w-fit rounded-full p-2 px-4 py-2 text-2xl items-center justify-start hover:bg-gray-900 ${item.functional?"cursor-pointer":"cursor-not-allowed"}`}><span className=''>{item.icon}</span><span className='text-lg tracking-wider'>{item.name}</span></Link>)}
        </div>
        <div className='flex w-full'>
          <button className='p-3 w-full mr-4 text-lg font-bold tracking-wider bg-sky-600 hover:bg-sky-700 rounded-full'>Post</button>
        </div>
